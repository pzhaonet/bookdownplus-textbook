...
I believe some official 'bookdown' templates will be available in the near future, but I cannot wait. Can you? I am afraid that the cake will not be fresh any more.
...
'bookdownplus' is the easiest shortcut to the world of 'bookdown'. With **just one single command** users can get a demo book (or multiple demo books) in .pdf or .doc, or even more formats (see Chapter \@ref(quick_start)). 'bookdownplus' extends the features of 'bookdown', and simplifies the procedure. Users only have to choose a template, clarify the book title and author name, and then focus on writing the text. No need to struggle in YAML and LaTeX.
...
By default, the book is built in a pdf file. From 'bookdownplus v1.0.3', users can get more output formats, including word (Microsoft Word), html (any web browser) and epub (for smart phones, or displayed by Calibre on desktop computers). From 'bookdownplus v1.2.0', uses can see the available output formats by running:
...
Furthermore, I have been developing some other packages, which bring more features intoF 'bookdown', such as:
